...
#--Projection space
inProj = Proj(init='epsg:2169')
outProj = Proj(init='epsg:4326')
#-- Name spaces
ns_citygml = 'http://www.opengis.net/citygml/2.0'
ns_gen = 'http://www.opengis.net/citygml/generics/2.0'
ns_citygml_generics = 'http://www.opengis.net/citygml/generics/2.0'
ns_cityobjectgroup = 'http://www.opengis.net/citygml/cityobjectgroup/2.0'
ns_appearance = 'http://www.opengis.net/citygml/appearance/2.0'
...
_geolocator = Nominatim(user_agent='nicolas.hoffmann@student.uibk.ac.at')#if many requests are sent it is important to specify the email address here

def add_lux_adress_data(gdf):
    # Load address data
    lux_adress_data = gpd.read_file("./data/addresses.geojson")
    lux_adress_data = lux_adress_data.to_crs("EPSG:4326")

    # Clip lux_adress_data using extent of gdf
    lux_adress_data = lux_adress_data.cx[gdf.total_bounds[0]:gdf.total_bounds[2], gdf.total_bounds[1]:gdf.total_bounds[3]]

    # Create Coordinates for roof data
    gdf['centroid'] = gdf['geometry'].centroid
    
    # Replace values with nearest neighbor from lux_adress_data
    for index, row in gdf.iterrows():
        nearest = nearest_points(row['centroid'], lux_adress_data['geometry'])
        nearest_index = nearest[1].distance(row['centroid']).idxmin()
        nearest_row = lux_adress_data.loc[nearest_index]
            
        gdf.at[index, 'City'] = nearest_row['localite']
        gdf.at[index, 'PostalCode'] = nearest_row['code_postal']
        gdf.at[index, 'Street'] = nearest_row['rue']
        gdf.at[index, 'StreetNumber'] = nearest_row['numero']
